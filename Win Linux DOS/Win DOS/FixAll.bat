...
ECHO   Q=打开或关闭测试模式
ECHO   R=启用或禁用数字签名验证（重启后依旧生效）
ECHO 请选择一项以继续：
CHOICE /C 123456789ABCDEFGHIJKLMNOPQR0
CLS
...
CHOICE /C YNC /M "Y=打开，N=关闭，C=取消"
IF %ERRORLEVEL%==3 (GOTO MAIN)
IF %ERRORLEVEL%==1 (BCDEDIT /SET TESTSIGNING ON) ELSE (BCDEDIT /SET TESTSIGNING OFF)
PAUSE
GOTO MAIN

:27
ECHO 启用或禁用数字签名验证（重启后依旧生效）
CHOICE /C YNC /M "Y=启用，N=禁用，C=取消"
IF %ERRORLEVEL%==3 (GOTO MAIN)
IF %ERRORLEVEL%==2 (BCDEDIT /SET NOINTEGRITYCHECKS ON) ELSE (BCDEDIT /SET NOINTEGRITYCHECKS OFF)
PAUSE
GOTO MAIN